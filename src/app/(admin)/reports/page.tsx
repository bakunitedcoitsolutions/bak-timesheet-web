...
import { reportOptions } from "@/utils/user.utility";

const ReportsPage = () => {
  const router = useRouter();
  const [hoveredCard, setHoveredCard] = useState<string | null>(null);
  const { privileges, isAdmin, isManager, isBranchManager, isLoading } =
    useAccess();

  const isFullAccess = isAdmin || isManager || isBranchManager;

  const allowedReports = isLoading
    ? []
    : isFullAccess
      ? reportOptions
      : reportOptions.filter((option) =>
          privileges?.reports?.items?.some(
            (item) => item.reportId === option.id && item.enabled
          )
        );
    ...
    <div className="flex flex-col gap-8 px-6 py-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-base text-gray-600 leading-relaxed">
          Access and generate various system reports and analytics
        </p>
      </div>

      {allowedReports.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 bg-gray-50/50 rounded-2xl border border-gray-100 mt-8 min-h-[400px]">
          <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center mb-6">
            <i className="fa-light fa-file-chart-column text-4xl text-primary"></i>
          </div>
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            No Reports Available
          </h3>
          <p className="text-gray-500 text-center max-w-sm">
            You currently do not have permission to view any reports. Please
            contact your administrator if you believe this is a mistake.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
          {allowedReports.map((option) => (
            <div
              key={option.id}
